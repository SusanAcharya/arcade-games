  ...
  const endGame = React.useCallback(
    async (winner: "player" | "bot") => {
      ...
      if (winner === "player") {
        ...
        // Award backend points and refresh header from server
        if (user) {
          try {
            await api.recordWin('dice-shootout', 50);
            const { data } = await api.myPoints();
            if (data?.total_points != null) setPlayerPts(data.total_points);
          } catch {}
        }
      } else {
        let add = 25;
    ...
    [playerClutchHeal, botClutchHeal, user]
  );

  // On mount, sync header points from server if logged in
  React.useEffect(() => {
    (async () => {
      if (!user) return;
      const { data } = await api.myPoints();
