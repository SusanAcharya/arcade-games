...
const GangWar21: React.FC = () => {
  ...
  // Resolve game
  const resolveGame = useCallback(async (finalDealerValue: number) => {
    ...
    setTimeout(async () => {
      if (playerScore > finalDealerValue) {
        ...
        // Award backend points & refresh server total
        if (user) {
          try {
            await api.recordWin('gang-war-21', 50);
            const { data } = await api.myPoints();
            if (data?.total_points != null) setPlayerPoints(data.total_points);
          } catch {}
        }
  ...
  }, [playerScore, playVictory, playDefeat, playTileClick, user]);
  ...
  // Initialize game and sync header points from server if logged in
  React.useEffect(() => {
    initializeDeck();
    (async () => {
      if (!user) return;
